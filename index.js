  ...
  if (matrixCanvas) {
    ...
    const drawMatrix = () => {
      ...
      for (let i = 0; i < drops.length; i++) {
        ...
        ctx.fillStyle = 'rgba(235, 215, 255, 0.98)'; // Vibranium purple matrix rain
        ctx.shadowBlur = 10;
        ctx.shadowColor = 'rgb(168, 85, 247)';
        ctx.fillText(text, i * fontSize, drops[i] * fontSize);
        
        ctx.fillStyle = Math.random() > 0.25 ? 'rgba(168, 85, 247, 0.55)' : 'rgba(99, 102, 241, 0.4)';
        ctx.shadowBlur = 2;
  ...
  if (canvas3D) {
    ...
    const render = () => {
      ...
      links.forEach(link => {
        ...
        if (link.type === 'rung') {
          // base rungs connect A (purple) and B (gold)
          const grad = ctx.createLinearGradient(p1.x, p1.y, p2.x, p2.y);
          grad.addColorStop(0, `rgba(168, 85, 247, ${alpha * 0.55})`);
          grad.addColorStop(1, `rgba(217, 119, 6, ${alpha * 0.55})`);
